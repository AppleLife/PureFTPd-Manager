...
- (void) _drawIconAtIndex:(unsigned int) index drawRect:(NSRect) drawRect {
	...
	//nameWidth = [[NSFont toolTipsFontOfSize:11.] widthOfString:name];
	...
	if (MacVersion < 0x1040){
		...
		drawPoint = NSMakePoint((buttonSize.width - nameSize.width)/2+NSMinX( buttonRect ), NSMaxY( buttonRect )- titleBaseline - nameHeight);
	}
...
- (BOOL) _dragIconImage:(NSImage *) iconImage andName:(NSString *) name andIdentifier:(NSString *) identifier event:(NSEvent *) event {
	...
	[dragImage lockFocus]; {
		...
		nameSize = [name sizeWithAttributes:attributesDictionary];
		nameHeight = [[NSFont toolTipsFontOfSize:11.] defaultLineHeightForFont];
		[name drawAtPoint:NSMakePoint((buttonSize.width - nameSize.width)/2, nameHeight + titleBaseline - nameSize.height ) withAttributes:attributesDictionary];
