...
//
// $Id: MCPConnection.h 49 2005-03-20 02:33:11Z glennsb $
// $Author: sergecohen $
...
@interface NSObject (SMySQLDelegate)
...
- (id) initToHost:(NSString *) host 
        withLogin:(NSString *) login 
         password:(NSString *) pass 
        usingPort:(int) port;

- (id) initToSocket:(NSString *) socket 
          withLogin:(NSString *) 
     login password:(NSString *) pass;
...
- (id) initToHost:(NSString *) host 
        ...
        usingPort:(int) port
          ...
          connect:(BOOL) doConnect;

- (BOOL) setConnectionOption:(int) option 
                     toValue:(BOOL) value;

- (BOOL) readMySQLDefaultGroup:(NSString *) groupName;


    // Port to 0 to use the default port
- (BOOL) connectWithLogin:(NSString *) login 
                 password:(NSString *) pass 
                     host:(NSString *) host 
                     port:(int) port 
                   socket:(NSString *) socket;
...
// Next method uses SHOW TABLES FROM db to be sure that the db is 
// not changed during this call.
